  ...
  pageWithSidebar(
    ...
    sidebarPanel(
      sliderInput("Transmission_Rate", "Select Transmission Rate: ",
                  min = 0, max = 2, value = 3.2, step = 0.05),
...
server <- shinyServer(
  function(input, output){
    ...
    output$curve <- renderPlot({
      ...
      parms <- c(transm = Transmission_Rate, recov = 0.23)
      times <- seq(from = 0,to = 40, by = 1/10)
      ...
      out %>%
        ...
        geom_line(size=2) + 
        ggtitle(label = "Coronavirus Curve" ,subtitle = paste0(transm_lab)) +
